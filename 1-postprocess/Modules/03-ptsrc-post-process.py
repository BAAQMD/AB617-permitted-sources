...
pm25_top10 = pm25_by_fac.sort_values(by='PM25_Ems_gs', ascending=False).reset_index(drop=True).iloc[:10].FacID.tolist()
for fac in pm25_top10:
...
tac_top10 = tac_by_fac.sort_values(by='TWE_Ems_gs', ascending=False).reset_index(drop=True).iloc[:10].FacID.tolist()
for fac in tac_top10:
    devs_fac = [dev for dev in devs if dev.split("-")[0]==fac]
    conc_fac_gdf = calc_conc_total(pm_emis_gs, tac_emis_gs, devs_fac)
